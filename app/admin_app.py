...
with col1:
    year = st.selectbox("対象年", ["2026", "2025", "2024"], index=0, disabled=st.session_state.is_running)
...
is_force = st.checkbox("⚠️ 既存データを上書きする (Force Rescrape)", help="既に収集済みのレースも含めて再取得します。欠損データの修復に有効ですが、時間がかかります。")
...
with tab_ml:
    ...
    with col_conf_1:
        ...
        if is_tuning:
            ...
            n_trials = st.slider("試行回数", 5, 100, 100)
        else:
    ...
    if start_process:
        ...
        if mode_val == "NAR":
            data_path = os.path.join(project_root, "ml", "processed_data_nar.parquet")
            db_path = os.path.join(project_root, "data", "raw", "database_nar.parquet")
            model_name = "lgbm_model_nar.pkl"
        else:
            data_path = os.path.join(project_root, "ml", "processed_data.parquet")
        ...
        if not skip_preprocess:
            with st.spinner("1/3 データ前処理中..."):
                if os.path.exists(db_path):
                   ...
                   st.error("database.parquetが見つかりません。")
                   st.stop()
        else:
...
with tab_data:
    ...
    uploaded_file = st.file_uploader("既存のデータベースCSV (database.csvなど) をアップロード", type=["csv"])
    ...
    if uploaded_file is not None:
        if st.button("変換して保存 (CSV -> Parquet)", type="primary"):
            ...
            if mode_val == "NAR":
                ...
                target_csv = os.path.join(project_root, "data", "raw", "database_nar.csv") # Backup
            else:
                target_parquet = os.path.join(project_root, "data", "raw", "database.parquet")
                target_csv = os.path.join(project_root, "data", "raw", "database.csv") # Backup

            with st.spinner("CSVを読み込み、Parquetに変換中..."):
                try:
                    # Save uploaded CSV temporarily? Or read directly.
                    # Read directly into pandas
                    df_upload = pd.read_csv(uploaded_file, low_memory=False, dtype={'horse_id': str, 'race_id': str})
                    
                    st.info(f"読み込み完了: {len(df_upload)} 行。変換中...")
                    ...
                    # Optionally save CSV backup if needed, but maybe not requested explicitly.
                    # Let's simple check "Use this data"
                    st.caption("このデータが「モデル学習」および「データ加工」で使用されます。")
