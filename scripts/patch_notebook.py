...
# The new content for the cell (split into lines)
NEW_CODE = r"""# 4.2 欠損データの補完 (HorseID & 過去走 & Metadata & Pedigree)
import pandas as pd
import sys
import os
import re
...
def patch_notebook():
    ...
    for cell in nb['cells']:
        if cell['cell_type'] == 'code':
            # Check if source starts with the header
            if cell['source'] and cell['source'][0].startswith("# 4.2 欠損データの補完"):
                # Convert NEW_CODE string to list of lines with \n
                new_source = [line + '\n' for line in NEW_CODE.split('\n')]
                # Remove last newline on last line if you want strict exactness, but python split usually handles it
                if new_source[-1] == '\n': new_source.pop()
                
                cell['source'] = new_source
                found = True
                print("Patched cell 4.2")
                break
    
    if not found:
        print("Could not find cell starting with '# 4.2 欠損データの補完'")
        return
