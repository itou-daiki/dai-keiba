...
def create_notebook():
    ...
    # --- Cell 1: Setup ---
    add_markdown_cell("# 🛠️ Scraper Error Fix Notebook\n\nこのノートブックは、スクレイピングのエラー（NameError, No History Data）を解消するための修正版コードです。\nまずはドライブをマウントし、必要なライブラリをセットアップします。")
...
""")
    ...
    # --- Cell 2: RaceScraper Class ---
    add_markdown_cell("## 1. RaceScraper Class Definition\n\nインポートエラーを防ぐため、RaceScraperクラスをここで直接定義します。")
    add_code_cell(scraper_code)

    # --- Cell 3: Wrapper Functions ---
    add_markdown_cell("## 2. Wrapper Functions (Fixing NameError)\n\nエラーの原因となっていた不足関数 (`fetch_race_horse_ids`, `fetch_horse_history`) を定義します。")
...
""")
    ...
    # --- Cell 4: NAR Backfill Script ---
    add_markdown_cell("## 3. NAR Data Fix Script\n\nNARデータ (`database_nar.csv`) の欠損IDと過去成績を補完します。")
    add_code_cell("""# NARコード
# 4.2 欠損データの補完 (Memory Efficient & Incremental Save - NAR)
...
def fill_missing_past_data_nar_notebook():
                    ...
                    for future in as_completed(futures):
                        try:
                            ...
                            if res:
                                rid, h_map = res
                                if h_map:
                                    # Update using vectorized map where possible or iter
                                    # Simple iter for safety
    ...
    # Skip Logic: Only horses that have NO past data yet
...
""")
    ...
    # --- Cell 5: JRA Backfill Script ---
    add_markdown_cell("## 4. JRA Data Fix Script (Debug)\n\nJRAデータ (`database.csv`) の過去成績を補完します。")
    add_code_cell("""# JRAコード
# 4.2 欠損データの補完 (Debug Version)
...
def fill_missing_past_data_jra_debug():
    ...
    for i in range(0, len(unique_horses), horse_batch_size):
        ...
        if history_store:
            ...
            print(f"  -> Batch Result: {updates_count} horses updated.")
...
""")
    ...
    # Save Notebook
    output_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'notebooks', 'Fix_Scraper_Errors.ipynb')
