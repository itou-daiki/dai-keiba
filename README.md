...
| 特徴量 | 説明 | デフォルト |
|-------|------|-----------|
| `turf_compatibility` | 芝コース平均着順 | 5.0 |
| `dirt_compatibility` | ダートコース平均着順 | 5.0 |
| `good_condition_avg` | 良馬場平均着順 | 10.0 |
| `heavy_condition_avg` | 重/不良馬場平均着順 | 10.0 |
| `distance_compatibility` | 距離±200m範囲平均着順 | 5.0 |
| `course_distance_record` | 会場×距離別平均着順（Rolling） | 10.0 |
| `best_similar_course_rank` | 類似コース最良着順 | 18.0 |

**リーク防止:**
```python
# course_distance_record の計算
df['course_distance_record'] = df.groupby('horse_course_key')['rank'].transform(
    lambda x: x.shift(1).expanding().mean()
)
# shift(1) により、当該レース結果は除外
```
...
| 特徴量 | 説明 | 値の範囲 |
|-------|------|----------|
| `is_rest_comeback` | 休養明けフラグ | 0 or 1（90日以上） |
| `interval_category` | 間隔カテゴリ | 1-4 |
| `is_consecutive` | 連闘フラグ | 0 or 1（14日以内） |
| `last_race_reliability` | 前走信頼度 | 0.4-1.0 |
...
| 特徴量 | 説明 | リーク防止 |
|-------|------|-----------|
| `jockey_compatibility` | 騎手との相性（同騎手での平均着順） | ✅ |
